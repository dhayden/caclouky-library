...
using (var scope = app.Services.CreateScope())
{
    ...
    // For SQLite: idempotently ensure migration history reflects the actual schema.
    // Runs on every startup; safe because each check is independent.
    if (string.IsNullOrEmpty(connectionString))
    {
        ...
        try
        {
            ...
            bool HasTable(string t) { cmd.CommandText = $"SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name='{t}'"; return (long)cmd.ExecuteScalar()! > 0; }
            bool HasCol(string t, string c) { cmd.CommandText = $"SELECT COUNT(*) FROM pragma_table_info('{t}') WHERE name='{c}'"; return (long)cmd.ExecuteScalar()! > 0; }
            bool InHistory(string id) { cmd.CommandText = $@"SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name='__EFMigrationsHistory'"; if ((long)cmd.ExecuteScalar()! == 0) return false; cmd.CommandText = $@"SELECT COUNT(*) FROM ""__EFMigrationsHistory"" WHERE ""MigrationId""='{id}'"; return (long)cmd.ExecuteScalar()! > 0; }
            void EnsureHistory() { cmd.CommandText = @"CREATE TABLE IF NOT EXISTS ""__EFMigrationsHistory"" (""MigrationId"" TEXT NOT NULL CONSTRAINT ""PK___EFMigrationsHistory"" PRIMARY KEY, ""ProductVersion"" TEXT NOT NULL)"; cmd.ExecuteNonQuery(); }
            void Mark(string id) { EnsureHistory(); if (!InHistory(id)) { cmd.CommandText = $@"INSERT INTO ""__EFMigrationsHistory"" VALUES ('{id}', '8.0.25')"; cmd.ExecuteNonQuery(); } }
